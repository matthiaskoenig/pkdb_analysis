...
def bubbles_data(studies_data, big_bubble_size, data_type="study"):
    if data_type == "study":
        studies_data["outputs_raw_count"] = (
            studies_data["outputs_count"] - studies_data["outputs_calculated_count"]
        )
        studies_data["group_not_individual"] = (
            studies_data["group_all_count"] - studies_data["individuals_count"]
        )
    ...
    for idx, study in studies_data.iterrows():
        ...
        if data_type == "study":
            raw_output = study_expand(
                study, "outputs_raw_count", big_bubble_size, "raw"
            )
            if len(raw_output) > 0:
                outputs = outputs.append(raw_output)

            calc_output = study_expand(
                study, "outputs_calculated_count", big_bubble_size, "calc"
            )
            ...
            group_subject = study_expand(
                study, "group_not_individual", big_bubble_size, "group_subject"
            )
            if len(group_subject) > 0:
                group_members = group_members.append(group_subject)

            individual = study_expand(
                study, "individuals_count", big_bubble_size, "individuals"
            )
            if len(individual) > 0:
        ...
        if data_type == "substance":
            ...
            raw_output = study_expand(
                study, "output_raw_number", big_bubble_size, "raw"
            )
            if len(raw_output) > 0:
                outputs = outputs.append(raw_output)

            calc_output = study_expand(
                study, "output_calculated_number", big_bubble_size, "calc"
            )
            if len(calc_output) > 0:
                outputs = outputs.append(calc_output)
...
def create_links(substances_data, studies_data, unused_substances=[]):
    substance_combinations_data = substance_combinations(
        studies_data, unused_substances
    )
    substance_com1 = pd.merge(
        substance_combinations_data,
        substances_data[["name", "start", "end", "label"]],
        how="inner",
        left_on="substance1",
        right_on="name",
    )
    substance_com1 = substance_com1.rename(
        columns={
            "start": "substance1_start",
            "end": "substance1_end",
            "label": "substance1_label",
        }
    )
    substance_com = pd.merge(
        substance_com1,
        substances_data[["name", "start", "end", "label"]],
        how="inner",
        left_on="substance2",
    ...
    substance_com = substance_com.rename(
        columns={
            "start": "substance2_start",
            "end": "substance2_end",
            "label": "substance2_label",
        }
    )
...
def create_config_files(studies_data, directory, data_type="study"):
    ...
    if data_type == "study":
        ...
        studies_data[["label", "start", "end", "interventions_count"]].to_csv(
            f"{directory}/data/intervention_number.txt",
            sep=" ",
            header=False,
            index=False,
        )
    ...
    if data_type == "substance":
        ...
        studies_data[["label", "start", "end", "name"]].to_csv(
            f"{directory}/data/{data_type}_names.txt",
            sep=" ",
            header=False,
            index=False,
        )
...
def substance_combinations(studies_data, unusubstances=[]):
    ...
    for study in studies_data.itertuples():
        if isinstance(study.substances, str):
            ...
            this_substance_combinations = set(
                [x.strip() for x in this_substance_combinations]
            )
            this_substance_combinations = this_substance_combinations - set(
                unusubstances
            )

            this_substance_combinations = iter(this_substance_combinations)
            substance_combinations = chain(
                substance_combinations, combinations(this_substance_combinations, 2)
            )
    ...
    substances_combinations = pd.DataFrame(
        substances_combinations_extended, columns=["substance1", "substance2", "study"]
    )
...
def substance_cooccurrence_matrix(studies_data):
    ...
    com = substances_combinations2.append(
        substance_combinations_data, ignore_index=True
    )
    coocurence_both_count = (
        com.groupby(["substance1", "substance2"]).occurance.count().reset_index()
    )
...
def add_label_and_type(study_data, substance_data, substance_to_type):
    study_data["type"] = study_data.substances.apply(
        find_study_type, substance_to_type=substance_to_type
    )
    substance_data["type"] = substance_data["name"].apply(
        lambda x: substance_to_type[x]
    )

    # type to label
    name_type = substance_data.loc[
        substance_data.groupby("type")["study_number"].idxmax()
    ][["name", "type"]]
    ...
    study_data["label"] = study_data.apply(
        find_label, type_to_label=type_to_label, axis=1
    )
    substance_data["label"] = substance_data.apply(
        find_label, type_to_label=type_to_label, axis=1
    )
