...
class LatexTables:
    ...
    def __init__(self, output_dir: Path,
        ...
        if len(substance_sids) != len(substance_shorts):
            raise ValueError(f"length of 'substance_sids' ({substance_sids}) must match length of short "
    ...
    def _create_latex_main(self):
        ...
        with open(self.output_dir / "tables.tex", "r") as f:
            ...
            latex = latex.replace(
                "SUBSTANCES",
                ", ".join([f"{sid} ({short})" for (sid, short) in
                           zip(self.substance_sids, self.substance_shorts)])
            )
    ...
    def _prepare_table(self, table_key) -> List[pd.DataFrame]:
        ...
        if table_key == "studies":
            ...
            for sid in self.substance_sids:
                del_keys = del_keys + [f'{sid}_plasma', f'{sid}_urine',
                                       f'{sid}_saliva']
            ...
            for column in df.columns:
                ...
                for k in self.replacement_order:
                    column = column.replace(self.substance_sids[k],
                                            self.substance_shorts[k])
        ...
        if table_key == "studies":
            ...
            for short in self.substance_shorts:
                for measurement in all_measurements:
                    df_new.loc[:, f'{short}_{measurement}'] = df.loc[:, f"{short}_{measurement}"]
            ...
            for k in range(len(self.substance_shorts) - 1):
                ...
                df_new.insert(loc=offset + (k * value), column=f"empty{k}",
                              value=[""] * len(df))
        ...
        for k, df in enumerate(dfs):
            ...
            if table_key == "timecourses":
                ...
                latex = latex.replace(
                    ...
                    "toprule\n\\rowcolor{white}\n " + \
                    " & & " + \
                    " & & ".join(["\multicolumn{" + str(n_cols) + "}{c}{"+sid+ " (" + short + ")}" for sid, short in zip(self.substance_sids, self.substance_shorts)]) + \
                    "\\\\\n" + \
                    " ".join(["\cmidrule{" + str(3+k*(n_cols+1)) + "-" + str(3+k*(n_cols+1)+(n_cols-1) )+ "}" for k in range(n_sids)]) + \
                    "\n\\rowcolor{white}"
                )

                # rotate headers
                for measurement in ["individual", "group", "error", "plasma", "urine", "saliva"]:
                    latex = latex.replace(measurement, r"\rotatebox{90}{" + measurement +'}')
