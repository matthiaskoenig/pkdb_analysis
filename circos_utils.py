...
def study_expand(study,  count_label, threshold, type_name = "raw"):
    ...
    if mega_size > 0:
        ...
        expanded_outputs = pd.concat([pd.DataFrame(mega_size_study).T]*mega_size, ignore_index=True)
...
def create_links(substances_data, studies_data,unused_substances=[]):
    ...
    substance_com1 = pd.merge(substance_combinations_data,substances_data[["name","start","end","label"]],how="inner",left_on="substance1", right_on ="name")
    substance_com1 = substance_com1.rename(columns = {"start":"substance1_start","end":"substance1_end","label":"substance1_label"})
    substance_com = pd.merge(substance_com1,substances_data[["name","start","end","label"]],how="inner",left_on="substance2", right_on ="name")
    substance_com = substance_com.rename(columns = {"start":"substance2_start","end":"substance2_end","label":"substance2_label"})
    ...
    return substance_com[["substance1_label","substance1_start","substance1_end","substance2_label", "substance2_start","substance2_end"]]
...
def create_config_files(studies_data, directory, data_type="study"):
    ...
    if data_type == "substance":
            ...
            studies_data[["label","start","end","name"]].to_csv(f"{directory}/data/{data_type}_names.txt", sep=" ",header=False,index=False)
            # all subjects number for number track
            studies_data[["label","start","end","study_number"]].to_csv(f"{directory}/data/study_number.txt", sep=" ",header=False,index=False)
            studies_data[["label","start","end","timecourse_number"]].to_csv(f"{directory}/data/timecourse_number.txt", sep=" ",header=False,index=False)
            studies_data[["label","start","end","output_number"]].to_csv(f"{directory}/data/output_number.txt", sep=" ",header=False,index=False)
            studies_data[["label","start","end","intervention_number"]].to_csv(f"{directory}/data/intervention_number.txt", sep=" ",header=False,index=False)
            #studies_data[["label","start","end","ones","fill_color"]].to_csv(f"{directory}/data/substance_pie.txt", sep=" ",header=False,index=False)
        ...
        if isinstance(study.substances,str):
            ...
            for substance_combination in substance_combinations:
                if len(substance_combination) > 0:
                    substances_combinations_extended.append(list(substance_combination) + [study_name])

    substances_combinations = pd.DataFrame(substances_combinations_extended,columns=["substance1","substance2","study"])
...
def substance_cooccurrence_matrix(studies_data, substances_data):
    ...
    coocurence_count = substance_combinations_data.groupby(["substance1","substance2"]).occurance.count().reset_index()
